...
# n<-20;Tm=100;frho=0.6;k=5; noCons<-T;nopois <- F
...
nlyzCW <- function(Tm,n,k,frho,noCons,nopois){
  ...
  repCW <- get(load(filename))[,(!noCons)+1]
...
overallnlyz<- function(TmVec,n,k,noCons){
  ...
  anlyzCW <- lapply(c(.2,.6), function(frho) t(sapply(TmVec, function(Tm)  nlyzCW(Tm,n,k,frho,noCons,T))))
  ...
  anlyzCW_P <- lapply(c(.2,.6), function(frho) t(sapply(TmVec, function(Tm)  nlyzCW(Tm,n,k,frho,noCons,F))))
